...
function ERF(alt, az, posture, I_dir, t_sol, f_svv, f_bes, asa) {
  ...
  //  f_svv : sky vault view fraction : fraction of sky vault in occupant view [0, 1]
  ...
  const ERF_direct = E_direct * (sw_abs / lw_abs);
  const dMRT_direct = ERF_direct / (hr * f_eff);

  const ERF_diff = E_diff * (sw_abs / lw_abs);
  const dMRT_diff = ERF_diff / (hr * f_eff);

  const ERF_refl = E_reflected * (sw_abs / lw_abs);
  const dMRT_refl = ERF_refl / (hr * f_eff);
